...
- `Settings` kini tidak lagi menampilkan card schedule terpisah; ringkasan `Legacy Global Schedule` dipindahkan ke area `Advanced Compatibility` agar tidak terlihat seperti editor kedua di [spo-backup-final/app/templates/settings.html](../spo-backup-final/app/templates/settings.html#L1)
...
- [x] Aktifkan runtime worker image terbaru setelah backup aktif selesai agar target selection berlaku penuh pada job berikutnya
...
- Web dan worker runtime kini sama-sama memakai image terbaru; refresh worker aman dilakukan pada Selasa, 28 Juli 2026 saat tidak ada task backup/download aktif

### 7b. Monitoring endpoint backup modern sempat terlalu berat untuk dipolling UI

- [x] Ringankan `/api/v2/backups` dan `/api/v2/backups/stats` agar tidak melakukan scan size rekursif penuh pada setiap load
- [x] Hapus fallback `live size` berbasis disk walk dari flow history normal saat counter byte task sudah tersedia
- [x] Compact payload `/api/task/active` dan `/api/backup/status/<tid>` agar list error besar tidak dikirim utuh pada setiap polling
- [x] Normalisasi progress metadata workload modern (`OneDrive`/`Outlook`/`Teams`) agar panel UI tidak lagi macet di `0 / 0` saat backup sebenarnya sudah berjalan
- [x] Simpan snapshot `PROGRESS` workload modern ke Redis agar reload halaman tidak kehilangan state task aktif
- [x] Tambahkan cache panel progress + fallback speed turunan di UI dashboard/history agar live bar tetap tampil walau `speed_human` backend belum sempat terisi
- [x] Sinkronkan state `paused` dari tracked task ke history modern dan dashboard queue
- [x] Tambahkan bulk selection + bulk delete pada halaman `Backups`
- [x] Tambahkan control aktif `Pause`, `Resume`, `Cancel` langsung pada halaman `Backups`
- [x] Tambahkan klasifikasi recovery `completed` vs `resume available` vs `retry recommended` vs `hard failed` pada history backup
- [~] Hardening pause modern di loop download Graph sudah aktif di runtime worker terbaru, tetapi masih perlu uji interruption file besar dan jaringan nyata

Detail:
- Sebelumnya halaman `Backups` bisa lama berhenti di `Loading backups...` karena endpoint history membaca size backup dengan path yang terlalu mahal untuk tenant backup besar
- Endpoint monitoring juga sempat lambat karena payload task aktif membawa array `errors` yang sangat besar dari backup OneDrive aktif
- Progress panel dashboard/history juga sempat terlihat salah untuk workload modern karena backend mengirim `users_count` / `targets_processed` tanpa kontrak progress yang diseragamkan ke UI lama SharePoint
- Setelah patch web runtime aktif pada `2026-07-28`, smoke test lokal menunjukkan `/api/v2/backups` dan `/api/v2/backups/stats` turun ke sekitar `~2 detik`, sedangkan `/api/task/active`, `/api/tasks/overview`, dan `/api/backup/status/<tid>` kembali merespons cepat
- Pada patch lanjutan yang diaktifkan pada Selasa, 28 Juli 2026, event `OneDrive` / `Outlook` / `Teams` kini ikut menulis snapshot progres ke Redis dan frontend menyimpan cache panel terakhir di browser, sehingga reload halaman tidak lagi langsung mengosongkan progress panel sebelum polling pertama balik
- Pada runtime yang sama, task aktif OneDrive `e935bd12-77bc-4f04-8b32-5777335e5b30` kini tampil sebagai satu project backup merge dengan status `paused`, `files_done=28`, `files_total=1230`, `current_site=hidpi`, dan `live_size=594.8 GB` alih-alih `0 / 0`
- History backup kini juga membawa `recovery_hint` dan CTA `Resume / Retry` yang menjalankan flow backup yang sama, sehingga operator tidak lagi perlu menebak mana run yang masih bisa dilanjutkan
...
- Raw editor kini dipisahkan ke halaman [spo-backup-final/app/templates/settings_advanced.html](../spo-backup-final/app/templates/settings_advanced.html#L1), sedangkan `/settings` fokus ke operator-safe surface seperti remote destinations, notification transport test, dan link ke flow utama
...
- [x] Perkaya resolusi owner backup legacy dari manifest/site metadata historis
...
Update:
- Backup registry sekarang mencoba memetakan backup `legacy` ke tenant compatibility dari konfigurasi aktif jika manifest lama memang belum ada
- Halaman `Backups` dan picker source di `Restore` kini menandai backup legacy hasil inferensi agar operator tahu mana owner yang pasti dari manifest dan mana yang hanya compatibility mapping

### 10b. Runtime worker modern belum terjamin aktif penuh hanya karena source code sudah dipatch

- [x] Verifikasi image/runtime `spo-backup-worker` sudah memakai patch terakhir setelah task aktif aman direstart
...
- [x] Modern workload backup (`OneDrive`, `Outlook`, `Teams`) kini me-reuse folder `backup_*` terakhir yang masih `running` / `interrupted` / `partial` / `failed` alih-alih selalu membuat folder timestamp baru
- [x] Modern workload backup kini menulis marker target selesai per user/team agar retry antar-run bisa skip target yang sudah tuntas
- [x] Backup engine kini mempertahankan satu folder kerja proyek per workload (`backup_current` untuk install baru, atau folder backup terakhir yang sudah ada untuk install lama) sehingga run sukses berikutnya tetap overwrite/merge ke proyek yang sama
- [x] Backup history API kini collapse banyak folder split lama menjadi satu project row per tenant/workload agar UI restore/history tidak terus menampilkan spam histori
...
- Sebelumnya backup modern selalu membuat folder baru seperti `.../onedrive/backup_YYYYMMDD_HHMMSS`, sehingga retry setelah disconnect memecah histori menjadi banyak backup kecil
- Sekarang retry modern workload akan berusaha menempel ke folder kerja terakhir dan melanjutkan target yang belum punya marker selesai
- Untuk data lama yang sudah terlanjur terpecah di banyak folder, UI kini menampilkan merged project view; konsolidasi fisik folder lama yang inactive tetap lebih aman dilakukan setelah task aktif selesai
...
- [x] Naikkan page size listing Graph, ukuran chunk download, dan HTTP pool agar transfer tidak terlalu banyak terpotong round-trip kecil
- [x] Batch flush manifest checkpoint agar site backup besar tidak menulis ulang manifest penuh pada setiap file selesai
...
- Patch throughput pada Selasa, 28 Juli 2026 menaikkan default `GRAPH_DOWNLOAD_CHUNK_SIZE` ke `4 MiB`, `GRAPH_LIST_PAGE_SIZE` ke `999`, `HTTP_POOL_*` ke `64`, dan menunda flush manifest ke checkpoint periodik (`25 file / 5 detik`) agar bandwidth lebih fokus ke transfer data daripada I/O metadata
- Patch throughput pada Rabu, 29 Juli 2026 menambahkan cache token `GraphAuth` di engine legacy dan memprioritaskan `@microsoft.graph.downloadUrl` ketika tersedia agar transfer file tidak perlu selalu membawa round-trip autentikasi tambahan ke endpoint `/content`
- Untuk workload modern, source backup OneDrive kini memakai project folder kerja yang sama saat resume dan juga memprioritaskan `@microsoft.graph.downloadUrl`; benchmark live tenant besar setelah patch ini masih perlu dibuktikan lagi agar angka throughput dan respons progress benar-benar stabil
...
- [x] Tambahkan fallback auto-dispatch dari endpoint `/api/task/active` dan `/api/tasks/overview` agar antrean backup bisa hidup lagi setelah restart walau tidak ada task finish event
- [x] Pastikan dashboard dan halaman backup mem-poll task aktif ulang supaya panel progress muncul tanpa reload manual saat backup baru mulai
...
- Web layer kini menambahkan fallback auto-dispatch saat polling `active` dan `overview` berjalan, sehingga antrean tetap maju walau worker belum direstart
- Smoke check pada Selasa, 28 Juli 2026 menunjukkan queued backup `107b49b0-94e2-4276-8707-f16fa7a41eeb` berhasil otomatis ter-dispatch ulang sebagai task aktif `54d9cbd5-bb76-4295-a3ba-2bcce19cc21e`
...
- [~] UX restore readiness sudah membaik, termasuk mode source tenant -> target tenant untuk cross-tenant copy, tetapi warning permission tenant dan hasil validasi tenant nyata belum menyeluruh
...
Update:
- Flow `Restore` modern kini memisahkan `source tenant` dan `target tenant`, sehingga backup tenant A bisa dipulihkan atau disalin ke tenant B tanpa mengubah engine backup dasar
- Permission preflight tetap dievaluasi terhadap target tenant, karena tenant itulah yang menerima write operation saat restore/copy dijalankan
- UI restore kini juga menandai secara eksplisit apakah operator sedang menjalankan `Restore` biasa atau `Cross-Tenant Copy`, termasuk ringkasan source tenant, target tenant, dan boundary credential pada preview/job history
- Validasi produksi masih perlu tenant kedua nyata untuk membuktikan skenario cross-tenant penuh di workload selain SharePoint
