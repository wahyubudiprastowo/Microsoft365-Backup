...
- [x] Refactor halaman settings menjadi section yang lebih aman
...
- [~] Semua layout backup harus terbaca dan diberi label benar, termasuk merged project view untuk split run lama
- [~] User harus bisa memahami backup milik tenant/workload mana tanpa ambiguity, tanpa dibanjiri banyak row untuk project yang sama
...
- [~] Engine backup/download mendukung skip unchanged, resume partial `.tmp`, progressive byte estimation, dan resume folder kerja modern antar-run; masih perlu matriks uji gangguan jaringan nyata
...
- [ ] Waktu uji end-to-end cross-tenant restore/copy pada tenant sumber dan tenant target non-produksi
...
- [x] Pisahkan advanced compatibility/raw config dari settings operasional harian
