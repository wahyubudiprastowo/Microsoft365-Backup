...
class OneDriveRestore(BaseRestore):
    ...
    def restore(self) -> dict:
        ...
        try:
            for user_dir in self.backup_path.iterdir():
                ...
                resolved_root = self.target_folder or "[drive root]"
                destination_plan.append({
                    "backup_source": backup_email,
                    "target_user": target_email,
                    "resolved_path": f"{target_email}:/{resolved_root}",
                })
    ...
    def dry_run(self) -> dict:
        result = {
            ...
            "files_to_upload": 0,
            "total_size_bytes": 0,
            "target_folder_requested": self.target_folder or None,
            "user_mapping_entries": len(self.user_mapping or {}),
            "resolved_destination_summary": f"OneDrive:/{self.target_folder}" if self.target_folder else "OneDrive:/[drive root]",
        }
        for user_dir in self.backup_path.iterdir():
            if not user_dir.is_dir() or user_dir.name.startswith("_"):
                continue
