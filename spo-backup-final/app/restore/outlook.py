...
class OutlookRestore(BaseRestore):
        ...
        try:
            for user_dir in self.backup_path.iterdir():
                ...
                destination_plan.append({
                    ...
                    "resolved_path": f"{target_email}:/mailbox/{RESTORE_FOLDER_NAME}",
                })
    ...
    def dry_run(self) -> dict:
        result = {
            ...
            "restore_items_requested": list(self.restore_items or []),
            "user_mapping_entries": len(self.user_mapping or {}),
            "resolved_destination_summary": f"Mailbox restore folder: {RESTORE_FOLDER_NAME}",
        }
        for user_dir in self.backup_path.iterdir():
            if not user_dir.is_dir() or user_dir.name.startswith("_"):
                continue
