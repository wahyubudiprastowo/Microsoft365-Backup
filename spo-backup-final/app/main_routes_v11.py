...
def register_v11_routes(app):
    ...
    def _apply_active_backup_overlay(backups: list) -> tuple[list, dict | None]:
        try:
            ...
            active_backup = get_active_backup_guard(fast=True)
        except Exception:
            active_backup = None
        ...
        for item in backups:
            ...
            control_state = str(active_backup.get("control_state") or "running").lower()
            item["status"] = control_state
            if control_state == "paused":
                item["recovery_state"] = "paused"
                item["recovery_label"] = "paused"
                item["recovery_hint"] = "This backup is paused. Use the live progress panel Resume button to continue the current task."
                item["resume_supported"] = True
            else:
                item["recovery_state"] = "active"
                item["recovery_label"] = "active task"
                item["recovery_hint"] = "This backup project is currently active. Use the live progress panel to pause, resume, or cancel it."
                item["resume_supported"] = False
            item["resume_action_kind"] = None
            item["resume_action_label"] = None
            break
    ...
    def list_backups_v2():
        ...
        try:
            ...
            active_backup = None
        if active_backup and not (((active_backup or {}).get("meta") or {}).get("backup_path")):
            active_backup = None
        active_path = ((active_backup or {}).get("meta") or {}).get("backup_path")
        backups = registry.list_all(
            use_cache=request.args.get("refresh") != "1",
            collapse_projects=True,
            prefer_backup_path=active_path,
    ...
    def backup_stats_v2():
        ...
        if active_backup and not (((active_backup or {}).get("meta") or {}).get("backup_path")):
            active_backup = None
        active_path = ((active_backup or {}).get("meta") or {}).get("backup_path")
        backups, active_backup = _apply_active_backup_overlay(
            registry.list_all(
                use_cache=request.args.get("refresh") != "1",
                collapse_projects=True,
                prefer_backup_path=active_path,
        ...
        stats = {
            ...
            "total_size_human": registry._format_size(total_size),
            "by_tenant": by_tenant,
            "by_workload": by_workload,
            "tenant_root": str(registry.tenant_root),
            "legacy_root": str(registry.legacy_root),
        }
        if active_backup:
            stats["active_backup"] = {
                "task_id": active_backup.get("task_id"),
