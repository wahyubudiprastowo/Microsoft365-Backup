...
def _store_task_snapshot(redis_client, task_id: str, state: str, meta: dict | None):
    try:
        ...
        redis_client.setex(f"spo:task:{task_id}:snapshot", 86400, json.dumps(payload, default=str))
    except Exception as e:
        log.warning(f"Failed to store task snapshot for {task_id[:8]}: {e}")
...
def run_backup_task(self, custom_root: str = None, tenant_id: str = None, workloads: list = None, site_paths: list = None):
    ...
    if site_paths:
        log.info(f"  Site scope     : {', '.join(str(item) for item in site_paths if str(item).strip())}")
    log.info("=" * 60)
    ...
    try:
        ...
        if "sharepoint" in workloads_to_run:
            ...
            sharepoint_stats = engine.run_backup(
                custom_root=sharepoint_root,
        ...
        if not stats.get("cancelled"):
            ...
            for workload_name, label, total_key, module_name, class_name in graph_workload_specs:
                ...
                def workload_progress_cb(evt, data, current_workload=workload_name, current_label=label):
                    ...
                    snapshot_meta = {
                        "event": f"{current_workload}_{evt}",
        ...
        partial_failures = bool(stats.get("failed_sites")) or bool([
            str(err) for err in stats.get("errors", [])
            if not str(err).startswith("Fatal:")
        ])
        task_failure = None
        if fatal_errors:
            stats["status"] = "failed"
            ...
            log.warning("BACKUP COMPLETED WITH PARTIAL FAILURES")
        else:
        ...
        _store_task_snapshot(r, task_id, "SUCCESS", stats)
        r.delete("spo:current_backup_task")
        r.delete(f"spo:task:{task_id}:control")
        _dispatch_after_backup_completion()
        if task_failure:
            _store_task_snapshot(r, task_id, "BACKUP_FAILED", stats)
        ...
        _store_task_snapshot(r, task_id, "FAILURE", stats)
        r.delete("spo:current_backup_task")
        r.delete(f"spo:task:{task_id}:control")
        _dispatch_after_backup_completion()
        raise
...
def download_custom_url_task(self, url: str, dest_dir: str = None):
    ...
    try:
        ...
        _store_task_snapshot(r, task_id, "FAILURE", {"error": str(e)})
        r.delete("spo:current_download_task")
        r.delete(f"spo:task:{task_id}:control")
        dispatch_next_queued_operation("download")
        raise
