...
def register_v13_routes(app):
    ...
    def create_restore_job_v13():
        ...
        try:
            ...
            queue_item = OperationQueue().enqueue(
                ...
                "restore_v2",
                {"job_id": job["id"]},
                f"{job.get('operation_label') or 'Restore'} {job.get('workload', 'job')}",
                (
                    f"{job.get('source_tenant_name') or 'Unknown source'}"
                    f" -> {job.get('target_tenant_name') or 'Unknown target'}"
                    f" · {job.get('source_backup') or ''}"
                ),
            )
    ...
    def list_sharepoint_target_libraries_v13():
        data = request.get_json(force=True, silent=True) or {}
        tenant_id = str(data.get("target_tenant_id") or data.get("tenant_id") or "").strip()
