...
class OneDriveWorkload(BaseWorkload):
    ...
    def list_targets(self):
        users = []
        try:
            for user in self._paginate(f"{self.GRAPH}/users?$select=id,displayName,userPrincipalName,mail,userType,accountEnabled"):
                if user.get("accountEnabled") is False:
                    continue
                if str(user.get("userType") or "").strip().lower() == "guest":
                    continue
                principal = user.get("userPrincipalName") or user.get("mail", "")
                if not principal:
                    continue
                users.append({
                    "id": user["id"],
    ...
    def backup(self):
        ...
        backup_path, resumed_existing = self._resolve_backup_dir(self.backup_root)
        self.stats["backup_path"] = str(backup_path)
        self.stats["resumed_existing_backup"] = resumed_existing
        self._update_run_manifest(backup_path, {
            "workload": "onedrive",
        ...
        self._emit("backup_start", {"backup_path": str(backup_path), "resumed_existing_backup": resumed_existing})
        ...
        try:
            ...
            for idx, user in enumerate(users, start=1):
                ...
                target_dir = backup_path / self._safe_user_dir(target_name)
                if self._is_target_completed(target_dir):
                    self.stats["targets_processed"] += 1
                    self._emit("target_done", {"target_name": target_name, "status": "skipped-resume"})
                    continue
                ...
                try:
                    ...
                    if self._is_unavailable_drive_error(e):
                        self.stats["targets_skipped"] += 1
                        self.stats["targets_unavailable"] += 1
                        self._emit("target_done", {"target_name": target_name, "status": "skipped-unavailable", "error": str(e)})
                        continue
        ...
        final_status = "cancelled" if self.stats["cancelled"] else ("failed" if any(str(err).startswith("Fatal:") for err in self.stats["errors"]) else ("partial" if self.stats["targets_failed"] else "success"))
        self._update_run_manifest(backup_path, {
            "status": final_status,
            "end_time": self.stats["end_time"],
            "targets_processed": self.stats["targets_processed"],
            "targets_failed": self.stats["targets_failed"],
            "targets_skipped": self.stats["targets_skipped"],
            "targets_unavailable": self.stats["targets_unavailable"],
    ...
    def _backup_user(self, user: dict, backup_path: Path):
        ...
        self._write_target_complete_marker(user_dir, {
            "target_type": "onedrive_user",
    ...
    def _walk_drive(self, drive_id: str, dest_dir: Path, files_index: list, item_id: str = None, relative_path: str = ""):
        ...
        for item in self._paginate(url, params={"$top": 200}):
            ...
            download_url = item.get("@microsoft.graph.downloadUrl") or f"{self.GRAPH}/drives/{drive_id}/items/{item['id']}/content"
    ...
    def _save_manifest(self, backup_path: Path):
        self._write_json(backup_path / "_workload_manifest.json", {
            ...
            "bytes_downloaded": self.stats["bytes_downloaded"],
            "errors": self.stats["errors"][:20],
            "status": "cancelled" if self.stats["cancelled"] else ("failed" if any(str(err).startswith("Fatal:") for err in self.stats["errors"]) else ("partial" if self.stats["targets_failed"] else "success")),
            "backup_path": str(backup_path),
            "start_time": self.stats["start_time"],
            "end_time": self.stats["end_time"],
            "resumed_existing_backup": bool(self.stats.get("resumed_existing_backup")),
        })
    ...
    def _is_unavailable_drive_error(self, exc: Exception) -> bool:
        if isinstance(exc, requests.HTTPError) and exc.response is not None:
            if exc.response.status_code in {404, 423}:
                return True
        message = str(exc)
        return "404 Client Error" in message or "423 Client Error" in message
